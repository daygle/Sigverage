...
import com.sigorage.app.BuildConfig
import com.sigorage.app.R
import com.sigorage.app.model.RecordingSchedule
import com.sigorage.app.model.SamplingMode
import com.sigorage.app.model.ThemeMode
...
fun SettingsScreen(
    ...
    // on screen. Earlier versions used early `return`s here - that silently
    // dropped ScheduleDialog (and any other dialog) when the Schedules page
    // was active, because the dialog block lived after the return.
    ...
    Column(
        ...
        // The rest of the runtime permissions + battery-optimisation exemption
        // + exact-alarm capability rows live together behind this single row.
        SettingsCard(
...
/**
 * Open the system battery-optimisation list so the user can mark Sigorage
...
// ---------------------------------------------------------------------------
// Permissions & Access sub-page (consolidates App Permissions + Background Access)
// ---------------------------------------------------------------------------

/**
 * Single drill-out that hosts two sub-sections:
 *  - **App Permissions** - the runtime manifest permissions rendered by
 *    [PermissionsSection] (location / notifications / activity recognition /
 *    phone state). Cheapest path: just drop the inner section in - it has
 *    its own internal Column with horizontal/vertical padding plus the
 *    spacedBy arrangement, so it nest cleanly under the SettingsCard header.
 *  - **Background Access** - the OS-managed toggles that previously lived
 *    as their own top-level SettingsCard in main settings (battery-opt
 *    exemption + exact-alarm capability). Owns its own ON_RESUME observer
 *    for [ReliabilityStatus] since this is the only place it's surfaced.
 */
...
private fun PermissionsAccessPage(onBack: () -> Unit) {
    ...
    // Re-read reliability on every Activity resume so system-settings changes
    // made in the middle of this drill-out are reflected immediately on
    // return.
    ...
    Scaffold(
        ...
        Column(
            ...
            // App Permissions (manifest runtime permissions)
            SettingsCard(
                headerIcon = Icons.Default.Security,
            ...
            // Background Access (OS-level toggles, deep-link only)
            SettingsCard(
